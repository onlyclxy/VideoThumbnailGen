...
def videoToImage():
    ...
    while not work.empty():
        ...
        try:
            ...
            numberOfSegments=float(totalSeconds/NumLength/NumWidth)
            numberOfSegments=round(numberOfSegments, 3)
            #a=time.strftime('%H:%M:%S', time.gmtime(1000))
            for x in range(NumLength*NumWidth):
...
def image_concat(image_names,filename,img_dir,image_des,videoFilepath,max_resolution=2560):
    ...
    target_shape = (NumLength*width, NumWidth*height)
    ...
    for ind, image_name in enumerate(image_names):
        ...
        if img.mode != "RGB":             # mode规整
            img = img.convert("RGB")
        row, col = ind//NumLength, ind%NumLength
...
if __name__ == '__main__':
    ...
    selectpath="C:\\test"
    selectpath=r"C:\test\474_Tie篝火_Lov64_Ser1063.mp4"

    NumLength=4
    NumWidth=4

    while True:
        user_input = input("请输入长和宽（例如4x3)忽略请直接按回车跳过,默认为4x4:")

        if not user_input:  # 如果用户没有输入，则跳过
            break

        match = re.match(r"(\d+)x(\d+)", user_input)

        if match:
            NumLength = int(match.group(1))
            NumWidth = int(match.group(2))
            print("输入的值为:%dx%d"%(NumLength,NumWidth))
            break
        else:
            print("输入格式不正确，请重新输入。")
